...
namespace Miniville
{
    class Game
    {
                ...
                for (int i = 0; i < listPlayer.Count; i++)
                {
                    foreach (var Cards in listPlayer[i].CardsAvailable)
                    {
                        if (Cards.Value.PileCards.Peek().ActivationValue.Item1 == ScoreDesTotal || Cards.Value.PileCards.Peek().ActivationValue.Item2 == ScoreDesTotal)
                        {
                            ...
                            else if (Cards.Value.PileCards.Peek().Type == (int)Card.Colorcard.Red && PlayerRound != i) Cards.Value.PileCards.Peek().ActiveEffect();
                        }
                    }
                }
        ...
        private bool hasGare(Player player)
        {
            ...
            {
                if (item.Value.PileCards.Peek().Name == "Gare" && item.Value.PileCards.Peek().IsActive /*Liste de monuments qu'on mettra plus tard j'sais pas*/) return true;
            }
            return false;
        }
        private bool hasTour(Player player)
        {
            foreach (var item in player.CardsAvailable)
            {
                if (item.Value.PileCards.Peek().Name == "Tour" && item.Value.PileCards.Peek().isActive/*Liste de monuments qu'on mettra plus tard j'sais pas*/) return true;
            }
            return false;
        }
        private bool hasParc(Player player)
        {
            foreach (var item in player.CardsAvailable)
            {
                if (item.Value.PileCards.Peek().Name == "Parc" && item.Value.PileCards.Peek().isActive/*Liste de monuments qu'on mettra plus tard j'sais pas*/) return true;
            }
            return false;
        }
        private bool hasCentre(Player player)
        {
            foreach (var item in player.CardsAvailable)
            {
                if (item.Key.piles.Peek().name == "CentreCommercial" && item.Key.piles.Peek().isActive/*Liste de monuments qu'on mettra plus tard j'sais pas*/) return true;
            }
            return false;
        }
